...
expect_snapshot(elic_add_data(x, data_source = round_1[, -1], round = 1),
                error = TRUE)
    ## ! mismatch between number of columns of source file and elicit object
...
expect_snapshot(elic_add_data(x, data_source = rbind(round_1, round_1),
                ...
                error = TRUE)
    ## ! mismatch between number of experts of source file and elicit object
...
expect_snapshot(out <- elic_add_data(y, data_source = z,
                ...
                error = TRUE)
    ## would it be possible to print the name of the expert instead of a number?
...
expect_snapshot(out <- elic_add_data(y, data_source = z,
                                     round = 2, verbose = FALSE),
                error = TRUE)
    ## very clear, but where do you set the data for round 1?
...
expect_snapshot(out <- elic_add_data(y, data_source = z,
                                     round = 2, verbose = FALSE),
                error = TRUE)
    ## clear but same issue as before, where do you set round 1?
