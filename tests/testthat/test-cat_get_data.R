test_that("Errors", {
  ...
  expect_snapshot(cat_get_data("abc", mechanism = "mechanism_1"),
                  error = TRUE)

  # When mechanism is not a character string
  expect_snapshot(cat_get_data(obj, mechanism = 1),
                  error = TRUE)
  ...
  expect_snapshot(cat_get_data(obj, mechanism = "mechanism_4"),
                  error = TRUE)

  # When site is not available in the object
  expect_snapshot(cat_get_data(obj, mechanism = "mechanism_1", site = "site_5"),
                  error = TRUE)

  # When site is not among the available sites in the object
  expect_snapshot(cat_get_data(obj, mechanism = "mechanism_3", site = "site_4"),
                  error = TRUE)
})
...
test_that("Output", {
  obj <- create_cat_obj()
  # Get all variables
  expect_identical(cat_get_data(obj, mechanism = "mechanism_1"),
                   obj[["data"]][["mechanism_1"]])
  expect_identical(cat_get_data(obj, mechanism = "mechanism_2"),
                   obj[["data"]][["mechanism_2"]])
  expect_identical(cat_get_data(obj, mechanism = "mechanism_3"),
